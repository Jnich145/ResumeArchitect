...
const SignIn = () => {
  ...
  const validateForm = () => {
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      setError(t('signIn.errors.invalidEmail') || 'Invalid email');
      return false;
    }
    if (!password) {
      setError(t('signIn.errors.passwordRequired') || 'Password is required');
      return false;
    }
    return true;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError((err as Error).message || t('signIn.error') || 'An error occurred');
    } finally {
      setIsLoading(false);
    }
  };
